...
degdist_exponents <- c(2, 2.2, 2.4, 2.6, 2.8, 3, 5, 10)
graphlist <- list()
matlist <- list()
for(e in 1:length(degdist_exponents)) {
    graphlist[[e]] <- sample_fitness_pl(no.of.nodes = nnodes, no.of.edges = nedges,
                                        exponent.out = degdist_exponents[e])
    matlist[[e]] <- as_adj(graphlist[[e]], type = "both", sparse = FALSE)

    ##names(graphlist) <- names(matlist) <- degdist_exponents
...
### Node Systems
    ...
    if(choice %in% c("pref_attach", "jpr", "lfr-n100k6-decayrate")) {# connection strength
        D <- 0.2 # production value is 0.20
...
### Main Loop
    ...
    df <- cbind(df, maxeig, maxsd, avgsd, maxac, avgac) # , moranI
    ...
    print(paste0("Power law exponent: ", degdist_exponents[e]))
    print(as.matrix(round(colMeans(kendalls[, columns], na.rm = TRUE), 3)))
    print(vcount(g))
    ...
    ht <- 5
    if(save_plots) {
        pdf(paste0("./img/earlywarnings-exp", degdist_exponents[e], ".pdf"),
            width = wd, height = ht)
    ...
    matplot(df$Ds, df[, columns], type = "p", pch = c(1, 3, 4), lwd = 1, col = colors,
            ...
            main = paste0("Power law exponent is ", degdist_exponents[e]))
