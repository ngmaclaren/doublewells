...
palette("Okabe-Ito")#conference val: "Tableau 10")
...
wd <- 8
if(plot_errorbars) {
    pchcex <- 2; pchs <- 1:3; lty <- 1; lwd <- 7.5
    plotvals$adjust <- plotvals$signal_ - ((plotvals$set_ - 2)/5)
} else {
    pchcex <- 2; pchs <- 1:3; lwd <- 3#4
}
if(save_plots) {
    svg("./img/kendall-corr-figure.svg", width = wd, height = ht, bg = "transparent")
...
plot(NULL, ylim = ylims, xlim = xlims, #xlim = c(min(lowers)*.9, max(uppers)*1.1),
     ylab = "", xlab = "Rank Correlation", yaxt = "n", bty = "n",
     cex.lab = 1.75, cex.axis = 1.75)
axis(side = 2, tick = FALSE, labels = yticklabels, at = max(plotvals$signal_):1, las = 1,
     cex.axis = 1.75)
##ypos <- 2 + c((1/3), 0, -(1/3))
if(plot_errorbars) {
    points(adjust ~ est, data = plotvals, pch = pchs, lwd = lwd, col = plotvals$set_+1, cex = pchcex)
    segments(x0 = plotvals$lower, x1 = plotvals$upper, y0 = plotvals$adjust,
             lty = 1, lwd = lwd, col = plotvals$set_+1)
    legend("topleft", legend = legendlabels, col = 1:3+1, bty = "n", lwd = lwd, lty = lty, pch = pchs)
