...
results <- lapply(
    results, function(m) {
        m <- vector(mode = "list", length = length(stresses))
        names(m) <- stresses
...
for(i in 1:length(matrices)) {
    ...
    for(j in 1:length(stresses)) {
        ...
        for(t in 1:T) {
            results[[i]][[j]][t, ] <- x
            x <- double_well_coupled(
                x, r1, r2, r3, D, A, dt, noise(length(x), noise_level), stress
            )
        }
    }
}
...
for(i in 1:nrows) {
    ...
    if(i == 4) legend("topleft", legend = as.character(stresses), lty = 1, lwd = 4, col = pal)
